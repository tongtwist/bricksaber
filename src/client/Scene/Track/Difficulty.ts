...
export interface ITrackDifficultyProps extends IPropsWithGUIOptions<ITrackDifficultyGUIProps> {
	readonly bpm: number
...
export class TrackDifficulty extends Group {
	...
	private _createLayers (): Array<TrackLayer> {
		...
		if (this._bm.beatmap) {
			...
			const layersElementsByTiming: { [timing: number]: Array<IBeatmapBlock | IBeatmapBomb | IBeatmapWall> } = {}
			...
			const orderedTimings: Array<number>
				= timings.sort((a: number, b: number) => a > b ? 1 : a < b ? -1 : 0 )
