...
import { GUIContainer, VR } from "../../Components";
import { Group, Trail } from "../../Templates";
import Body from "./Body";
import Head from "./Head";
import LeftSaber from "./LeftSaber";
import RightSaber from "./RightSaber";
import type Scene from "..";
import { Cube } from "../Track/Cube";
...
export default class Player extends Group {
  ...
  private constructor(props: IPlayerProps) {
    ...
    const steps = 3;
    const sensorsNum = 3;
    this._leftTrail = new Trail({
      name: "leftTrail",
      steps,
      color: 0xff0000,
      sensorsNum,
    });
    this._rightTrail = new Trail({
      name: "rightTrail",
      steps,
      color: 0x0000ff,
      sensorsNum,
    });
    this._obj3D.add(this._leftTrail.group, this._rightTrail.group);
  }
  ...
  private _findWrappedObjectAndMakeItExplode(object: Mesh) {
    const wrappedObject = this._scene.getObjectById(object.id) as Cube;
    if (wrappedObject && wrappedObject.explode) {
      wrappedObject.explode();
    }
  }
  ...
  renderingComputation(t: number, dt: number, audioTime: number): void {
    if (this._leftSaber && this._leftSaber.pit) {
      ...
      for (const i of bladeCollisions) {
        ...
        m.color.set(0);
        this._findWrappedObjectAndMakeItExplode(o);
      }
      for (const a of trailCollisions) {
        for (const i of a) {
          const o: Mesh = i.object as Mesh;
          let m: MeshBasicMaterial = o.material as MeshBasicMaterial;
          // TODO change is already hit condition
          if (m.color.getHex() !== 0) {
            o.material = m.clone();
            m = o.material as MeshBasicMaterial;
            m.color.set(0);
  ...
  static async create(props: IPlayerProps): Promise<Player> {
