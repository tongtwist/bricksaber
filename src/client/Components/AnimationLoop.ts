...
export default class AnimationLoop {
  ...
  private _dtSinceLastAudioTime: number = 0;
  ...
  get timeAccelerator(): number {
    return this._timeAccelerator;
  }
  set timeAccelerator(v: number) {
    this._timeAccelerator = Math.max(0, v);
  }
  get started() {
  ...
  private _animate() {
    ...
    if (audioTime === this._oldAudioTime) {
      // console.log("same audio time")
      audioTime += dt;
      audioTime += this._dtSinceLastAudioTime;
      this._dtSinceLastAudioTime += dt;
    } else {
      // console.log("new")
      this._dtSinceLastAudioTime = 0;
